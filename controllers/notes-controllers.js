...
async function addNote(req, res) {
  ...
  if (text.length > 300) {
    return res.status(400).json({ error: "The title of the note must be at most 300 characters long" });
  }
  ...
  try {
    ...
    const newNote = await createNote(id, title, text, createdAt, modifiedAt);
